...
function verifyAcres() {
   try {
         if (acresBox.value > 0) {
            ...
            throw "Acres must be a positive number.";
         }
      }
   catch(message) {
      acresComplete = false;
      acresBox.value = ""; // remove input box value
...
function verifyMonths() {
   try {
      if (monthsBox.value > 0 && monthsBox.value < 13) {
         monthsComplete = true;
         messageElement.innerHTML = ""; // clear previous message or recommendation
         testFormCompleteness();
      } else {
   ...
   monthsBox.value = ""; // remove input box value
   messageHeadElement.innerHTML = ""; // remove any former recomendation heading
   messageElement.innerHTML = message; // display error message
}
}
