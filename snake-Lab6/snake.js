...
class Game {
    ...
    restart() {
        ...
        // Determines which caste was picked in settings manual and creates
        // the appropriate class or extended class
    ...
    update() {
        ...
        if ( this.isGameOver() ) { 
            ...
            // Moves all snakes created objects
            this.snake.slither()

            // Is the snakes head on the food
            if ( this.snake.isHeadOn(this.food) ) {
                ...
                // Snake grows when head is on food
                // Check for null for Kss'ish to grow slower
...
class KssishSnake extends Snake {
        ...
        grow() {
            ...
            if (Math.floor(Math.random()*2) === 1) {
                //super.grow() // Doesn't work, copied whole grow() into here.
                this.segments[this.segments.length - 1].untail()
                this.newSnakeSeg = new SnakeSegment(this.color, this.segments[this.segments.length - 1].gridPosition, null, false, true )
                this.segments.push(this.newSnakeSeg)
                return this.newSnakeSeg
